...
public class BlockVillageBoxSmall extends Block {

    protected BlockVillageBoxSmall() {
        super(Material.wood);
        this.setBlockName("BlockVillageBoxSmall");
    ...
    public void registerBlockIcons(IIconRegister reg) {
    	this.blockIcon = reg.registerIcon(Main.MODID + ":BlockVillageBoxSmall_Sides");
      	this.frontIcon = reg.registerIcon(Main.MODID + ":BlockVillageBoxSmall_Front");
      	this.topIcon = reg.registerIcon(Main.MODID + ":BlockVillageBoxSmall_Top");		
    }
	...
	public boolean onBlockActivated(World world, int x, int y, int z,EntityPlayer player, int p_149727_6_, float p_149727_7_, float p_149727_8_, float p_149727_9_) {
		...
		if(world.isRemote) return true;
			...
			int bx = x;							//x
			int bz = z;							//y
			int bsx = BuildingConfig.GroundWorkSmallSizeX;						//size x
			int bsz = BuildingConfig.GroundWorkSmallSizeZ;						//size z
			int by = y - 1;						//y
			int bt = BuildingData.Type_GroundworkSmall;							//type
