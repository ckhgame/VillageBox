...
public class PageProList extends Page{
	...
	public PageProList(GuiGuideBook guiGuideBook, int pageIdx) {
		super(guiGuideBook, StatCollector.translateToLocal(PathHelper.full("guideBook.professions")));
	...
	public void onInit() {		
		...
		if(this.pageIdx > 0){
			this.addLink(new LinkImg(this, this.guiGuideBook.guiBookGuiTextures,208,0,0,16,16,16,"prolist=" + (this.pageIdx - 1),null),top,0);
		}
		int pageCount = (pros.size() - 1) / prosPerPage + 1;
		if(this.pageIdx < pageCount - 1){
			this.addLink(new LinkImg(this,this.guiGuideBook.guiBookGuiTextures,192,0,0,16,16,16,"prolist=" + (this.pageIdx + 1),null),top,2);
		}
